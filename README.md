...
| Compute | EC2 (x2, t3.micro) | Hosts the Flask frontend |
| Load Balancing | Application Load Balancer | Distributes traffic across both AZs |
...
- **Comprehend scoped to `Resource: "*"`, consistent with a known service limitation** — `DetectSentiment` and `DetectKeyPhrases` don't support resource-level ARN scoping, the same constraint already documented for SNS in an earlier project in this series. Everything else keeps exact-ARN scoping.
- **No new network exposure** — Comprehend is called entirely from inside the Lambda functions over the AWS SDK, on AWS's internal network. Adding it doesn't touch the EC2/ALB/VPC security posture at all.
- **Same least-privilege IAM pattern as the original project**, just extended with the two Comprehend actions actually used.
...
- **[STEPS.md](STEPS.md)** — full build log, focused on what's different from the original project
- **[CONCEPTS.md](CONCEPTS.md)** — design rationale for the Comprehend integration and its security/cost implications
